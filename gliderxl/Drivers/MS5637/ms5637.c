...
#define MS5637_ADDR													0xEC //0b1110110
...
bool ms5637_is_connected(void)
{
	...
	i2c_stat = HAL_I2C_Master_Transmit(&hi2c2, MS5637_ADDR, NULL, 0, 100);
	if( i2c_stat != HAL_OK)
		return false;
...
enum ms5637_status ms5637_write_command( uint8_t cmd)
{
	...
	/* Do the transfer */
	i2c_stat = HAL_I2C_Master_Transmit(&hi2c2, MS5637_ADDR, data, 1, 100);
	if( i2c_stat == HAL_TIMEOUT )
...
enum ms5637_status ms5637_read_eeprom_coeff(uint8_t command, uint16_t *coeff)
{
	...
	i2c_stat = HAL_I2C_Master_Receive(&hi2c2, MS5637_ADDR, buffer, 2, 100);
	if( i2c_stat == HAL_TIMEOUT )
		return ms5637_status_no_i2c_acknowledge;
	if( i2c_stat != HAL_OK)
		return ms5637_status_i2c_transfer_error;
...
static enum ms5637_status ms5637_conversion_and_read_adc(uint8_t cmd, uint32_t *adc)
{
	...
	// delay conversion depending on resolution TODO: FreeRTOS delay
	HAL_Delay( conversion_time[ (cmd & MS5637_CONVERSION_OSR_MASK)/2 ]/1000 );
	if( status != ms5637_status_ok)
		return status;
	...
	i2c_stat = HAL_I2C_Master_Receive(&hi2c2, MS5637_ADDR, buffer, 3, 100);
	if( i2c_stat == HAL_TIMEOUT )
		return ms5637_status_no_i2c_acknowledge;
	if( i2c_stat != HAL_OK)
		return ms5637_status_i2c_transfer_error;
