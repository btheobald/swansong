...
void ICM_readBytes(uint8_t reg, uint8_t *pData, uint16_t Size) // ***
{
	...
	HAL_SPI_Transmit(SPI_BUS, &reg, 1, 100);
	HAL_SPI_Receive(SPI_BUS, pData, Size, 100);
...
void ICM_WriteBytes(uint8_t reg, uint8_t *pData, uint16_t Size) // ***
{
	...
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(SPI_BUS, &reg, 1, 100);
	HAL_SPI_Transmit(SPI_BUS, pData, Size, 100);
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_SET);
...
void ICM_ReadOneByte(uint8_t reg, uint8_t* pData) // ***
{
	...
	while (HAL_SPI_GetState(SPI_BUS) != HAL_SPI_STATE_READY)
		;
	HAL_SPI_Receive(SPI_BUS, pData, 1, 100);
	while (HAL_SPI_GetState(SPI_BUS) != HAL_SPI_STATE_READY)
		;
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_SET);
}
...
void ICM_WriteOneByte(uint8_t reg, uint8_t Data) // ***
{
	...
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(SPI_BUS, &reg, 1, 100);
	HAL_SPI_Transmit(SPI_BUS, &Data, 1, 100);
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_SET);
}
...
void ICM_PowerOn(void) {
	...
	if (test == whoami) {
		...
		myprintf("Failed WHO_AM_I.  %i is not 0xEA\r\n", test);
...
void ICM_CSHigh(void) {
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, SET);
}
void ICM_CSLow(void) {
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, RESET);
}
